...
void config_defaults(config_t *cfg)
{
    ...
    cfg->foreground = 0;
    cfg->auto_start = 1;
    /* Выключен по умолчанию, как у HydraRoute: он метит IPv6 только при
       глобальном адресе v6 на br0. Без v6 у провайдера правила и наборы
       v6 — лишняя работа и лишние отличия. */
    cfg->ipv6       = 0;
...
int config_write_default(const char *path)
{
    ...
    fprintf(f,
        ...
        "interface=%s\n"
        "\n"
        "# Обслуживать ли IPv6. Включать, только если провайдер даёт IPv6:\n"
        "# без него правила и наборы v6 — пустая работа.\n"
        "ipv6=no\n"
        "\n"
