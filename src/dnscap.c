...
/* Фильтр ядра: UDP с портом источника 53, IPv4 и IPv6, фрагменты
   отброшены. Байткод получен из `tcpdump -dd 'udp src port 53'` для
   канала Ethernet — писать его руками слишком легко ошибиться.
...
int dcap_open(dcap_t *c, const char *iface, char *err, unsigned err_size)
{
    ...
    /* SOCK_DGRAM: ядро само снимает канальный заголовок, и до нас
       доходит уже IP-пакет. Фильтр при этом всё равно видит кадр
       целиком, поэтому байткод рассчитан на Ethernet. */
