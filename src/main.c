...
static void usage(FILE *out)
{
    fprintf(out,
        ...
        "  -s, --status          показать состояние службы и выйти\n"
...
/* Закрываем всё, что досталось по наследству от того, кто нас запустил.

   Служба не должна зависеть от его аккуратности. Однажды обновление
   передало новой службе слушающий сокет прежней по цепочке
   sh -> opkg -> postinst: порт был занят чужой копией, свой bind падал
   с «Address in use», а соединения копились в очереди сокета, с
   которого никто не принимал. Снаружи это выглядело как «интерфейс не
   открывается», и починить это в предыдущей версии было уже нельзя —
   обновление выполняет именно она. */
