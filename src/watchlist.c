...
static int add_cidr(wl_t *w, int group, const char *raw)
{
    ...
    if (slash) {
        *slash = '\0';
        ...
        /* «10.0.0.0/» — опечатка или обрезанная строка. strtol на пустой
           строке отдаёт 0 и не жалуется, и запись становилась /0, то
           есть весь интернет в туннель, без единого слова в журнале.
           Цифра после косой черты обязательна, и /0 не принимаем
           вовсе: намеренно так не пишут, а цена промаха — всё. */
        if (!end || end == slash + 1 || *end || p <= 0) return -1;
        prefix = (int)p;
    }
