...
# 64 КБ: логотип 192×192 в стеклянной отрисовке весит 57 — резкости
# хватает и на экран входа, а на прошивке это одна сотая ядра Xray.
SIZE=$(wc -c < "$LOGO" | tr -d ' ')
if [ "$SIZE" -gt 64000 ]; then
    echo "web/logo.png весит $SIZE байт — уменьши до 192×192" >&2
    exit 1
fi
