...
#
# `opkg remove` сам по себе оставляет за собой файлы настроек (пакету
# они не принадлежат: создаются при первой установке), журнал и запись
# о репозитории. Для установки начисто этого мало.
